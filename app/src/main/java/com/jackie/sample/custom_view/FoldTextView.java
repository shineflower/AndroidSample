...
public class FoldTextView extends AppCompatTextView implements View.OnClickListener {
    ...
    public FoldTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            mShowMaxLine = typedArray.getInt(R.styleable.FoldTextView_showMaxLine, MAX_LINE);
            mTipGravity = typedArray.getInt(R.styleable.FoldTextView_tipGravity, END);
            mTipColor = typedArray.getColor(R.styleable.FoldTextView_tipColor, TIP_COLOR);
            ...
            isParentClick = typedArray.getBoolean(R.styleable.FoldTextView_isSetParentClick, false);
    ...
    private void addTip(SpannableStringBuilder span,  BufferType type) {
        if (!(isExpand && !isShowTipAfterExpand)) {
            ...
            span.setSpan(new ForegroundColorSpan(mTipColor),
                    span.length() - length,
                    span.length(),
    ...
    private void translateText(Layout layout, BufferType type) {
        if (layout.getLineCount() > mShowMaxLine) {
            ...
            if (mTipGravity == END) {
                ...
                end -= paint.breakText(mOriginalText,
                        start,
                        end,
                        false,
                ...
                while (layout.getPrimaryHorizontal(end - 1) +
                        getTextWidth(mOriginalText.subSequence(end - 1, end).toString()) < x) {
                    end++;
                }
                        ...
                        false,
                        paint.measureText(builder.toString()), null) + 1;
            }
