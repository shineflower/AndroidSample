package com.jackie.sample.card_swipe.view;
...
import com.jackie.sample.card_swipe.listener.FlingCardListener;
...
public class SwipeFlingAdapterView extends BaseFlingAdapterView {
    ...
    public SwipeFlingAdapterView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SwipeFlingAdapterView, defStyle, 0);
        yOffsetStep = ta.getDimensionPixelOffset(R.styleable.SwipeFlingAdapterView_y_offset_step, 0);
        ROTATION_DEGREES = ta.getFloat(R.styleable.SwipeFlingAdapterView_rotation_degrees, ROTATION_DEGREES);
        MIN_ADAPTER_STACK = ta.getInt(R.styleable.SwipeFlingAdapterView_min_adapter_stack, MIN_ADAPTER_STACK);
        MAX_VISIBLE = ta.getInt(R.styleable.SwipeFlingAdapterView_max_visible, MAX_VISIBLE);
        ta.recycle();
    ...
    public void init(final Context context, Adapter mAdapter) {
        if (context instanceof OnFlingListener) {
            mOnFlingListener = (OnFlingListener) context;
        } else {
...
//                removeAllViewsInLayout();
        ...
        if (adapterCount < MIN_ADAPTER_STACK) {
            if (mOnFlingListener != null) {
                mOnFlingListener.onAdapterAboutToEmpty(adapterCount);
            }
        }
    }
    ...
    private void setTopView() {
        if(getChildCount()>0){
            ...
            if(mActiveCard != null && mOnFlingListener != null) {
                ...
                mFlingCardListener = new FlingCardListener(mActiveCard, mAdapter.getItem(0),
                    ...
                    public void onScroll(float progress, float scrollXProgress) {
                        ...
                        mOnFlingListener.onScroll(progress, scrollXProgress);
                    }
                });

                // 设置是否支持左右滑
                mFlingCardListener.setIsNeedSwipe(isNeedSwipe);
    ...
    public FlingCardListener getTopCardListener() throws NullPointerException {
        if(mFlingCardListener ==null){
            throw new NullPointerException("mFlingCardListener is null");
        }
